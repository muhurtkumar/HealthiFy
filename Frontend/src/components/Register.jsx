    ...
    <Box sx={{ minHeight: "calc(100vh - 64px)", display: "flex", alignItems: { xs: "flex-start", sm: "center" }, justifyContent: "center", background: "#f9fafb", padding: { xs: 2, sm: 4 }, overflow: "auto" }}>
      <Box sx={{ backgroundColor: "#fff", boxShadow: 3, padding: { xs: 3, sm: 4 }, borderRadius: 3, maxWidth: 400, width: "90%", textAlign: "center", mt: { xs: 6, sm: 0 } }}>
        ...
        {["name", "email", "password"].map((field, index) => (
          <TextField key={index} fullWidth label={field === "name" ? "Full Name" : field === "email" ? "Email ID" : "Password"} name={field} type={field === "password" ? "password" : "text"} value={formData[field]} onChange={handleChange} margin="normal" variant="outlined"
            sx={{ "& .MuiOutlinedInput-root": { borderRadius: 20, height: 40, backgroundColor: "#e0f2fe" }, "& .MuiInputBase-input": { padding: "10px" } }}
            InputProps={{ startAdornment: <InputAdornment position="start">{field === "name" ? <PersonIcon sx={{ color: "#6b7280" }} /> : field === "email" ? <EmailIcon sx={{ color: "#6b7280" }} /> : <LockIcon sx={{ color: "#6b7280" }} />}</InputAdornment> }} />
        ))}
        ...
        <Typography sx={{ color: "#6b7280", mt: 2 }}>Already have an account? <span style={{ color: "#3b82f6", cursor: "pointer" }} onClick={handleSignUp}>Login</span></Typography>
      </Box>
    </Box>
  );
};
